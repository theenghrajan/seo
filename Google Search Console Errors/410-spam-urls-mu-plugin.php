...
add_action( 'template_redirect', function () {
	...
	// Query-string variant of the same injection: WordPress's own
	// redirect_canonical() (template_redirect priority 10, i.e. AFTER this
	// hook) 301-redirects malformed requests like /index.php?dp/003160797
	// to /?dp%2F003160797 and then serves 200 on that target - so both the
	// original raw-slash request shape and WP's own re-encoded redirect
	// target need to be caught here, before redirect_canonical ever runs.
	$query = parse_url( $_SERVER['REQUEST_URI'], PHP_URL_QUERY );

	if ( $query ) {
		// rawurldecode (not urldecode) so a literal '+' isn't also turned
		// into a space - we only care about resolving %2F back to '/'.
		$query_decoded = rawurldecode( $query );
		...
		foreach ( $query_patterns as $pattern ) {
			if ( preg_match( $pattern, $query_decoded ) ) {
				pmac_410_gone();
			}
		}
	}
